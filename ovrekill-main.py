...
def process_interval(test):
    ...
    logging.info(f"Test {name}: Converted interval {test['interval']} to cron {cron_string}")
...
def process_cron(test, ver, filename):
    ...
    if version.parse(ver) < version.parse('4.12'):
        if cron_category not in ['Bi-weekly any day', 'Monthly']:
            logging.info(f"Test {name}: Updated older version {ver} to be either bi-weekly or monthly with cron {cron_string}")
    ...
    # Decision based on cron_category
    if ver == '4.13' and cron_category != 'Weekly on specific day(s)':
        logging.info(f"Test {name}: 4.13 version detected. Updating cron to weekly-weekend: {cron_string}")
        changes_made.append("updated_4.13")
        del test['cron']
        test["cron"] = generate_cron_string('weekly-weekend')
    elif cron_category in ['Daily', 'Less than bi-weekly']:
        logging.info(f"Test {name}: Replaced cron {test['cron']} based on its category {cron_category} with {cron_string}")
...
def replace(test, ver, filename):
    ...
    if changes_made:
        logging.info(f"Test {name}: Changes made - {', '.join(changes_made)}")
